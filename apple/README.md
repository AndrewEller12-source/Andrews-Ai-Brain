...
Open `AiTaskManager.xcodeproj` in Xcode. The shared **Mac** and **iPhone** schemes build the native shells. The **Dashboard source (edit here)** group points to the actual JavaScript, HTML, CSS, backend, and root documentation; the generated `Runtime` resource is a staged copy and is overwritten during staging.
...
```sh
xcrun swiftc apple/Shared/Pairing.swift apple/Tests/PairingSmoke.swift -o /tmp/task-manager-pairing-smoke
/tmp/task-manager-pairing-smoke
xcodebuild -project apple/AiTaskManager.xcodeproj -scheme Mac -configuration Release -derivedDataPath apple/build/Mac CODE_SIGNING_ALLOWED=YES CODE_SIGN_IDENTITY=- build
xcodebuild -project apple/AiTaskManager.xcodeproj -scheme iPhone -destination 'generic/platform=iOS Simulator' -derivedDataPath apple/build/iPhone CODE_SIGNING_ALLOWED=NO build
```
