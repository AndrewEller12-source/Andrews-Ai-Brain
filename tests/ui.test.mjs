...
test('desktop questions are discoverable in attention and choice buttons fill an editable answer',async()=>{
 const thread={id:'desktop-question',title:'Design the menu',department:'Engineering',recordedStatus:'idle'},question={id:'desktop:q:42',threadId:thread.id,source:'desktop',method:'item/tool/requestUserInput',params:{questions:[{id:'color',question:'Which color?',options:[{label:'Blue',description:'Calm'}]}]}};
 const state=snapshot({threads:[thread],approvals:[question]});const a=app(state,(url)=>url==='/api/state'?snapshot({threads:[thread]}):{ok:true});a.$('#questions-open').click();assert.match(a.$('#content').textContent,/Design the menu/);a.$('[data-answer-choice]').click();assert.equal(a.$('[data-answer="color"]').value,'Blue');a.$('[data-answer="color"]').value='Blue with white text';a.$('[data-decision="accept"]').click();await tick();assert.deepEqual(a.calls.find(c=>c.url==='/api/approval').body.answers,{color:'Blue with white text'});a.close();
});
test('explicit steering sends exact turn and uncertain delivery keeps draft without intake fallback',async()=>{
 const thread={id:'steering',title:'Build storefront',department:'Engineering',activity:{source:'desktop',state:'running',turnId:'active-turn'}};const state=snapshot({threads:[thread],desktop:{connected:true}});const a=app(state,(url)=>url==='/api/steer'?{status:'uncertain',error:'Not confirmed'}:state);
 a.$('[data-task="steering"]').dispatchEvent(new a.w.Event('click'));a.$('#continue-task').click();assert.equal(a.$('#delivery').hidden,false);a.$('#delivery').value='steer';a.$('#delivery').dispatchEvent(new a.w.Event('change'));assert.match(a.$('#send').textContent,/Steer now/);a.$('#prompt').value='Use a darker background';a.$('#send').click();await tick();assert.equal(a.calls.find(c=>c.url==='/api/steer').body.expectedTurnId,'active-turn');assert.equal(a.calls.some(c=>c.url==='/api/intake'),false);assert.equal(a.$('#prompt').value,'Use a darker background');a.close();
});
test('inbox excludes completed work and waiting requests expose send now',async()=>{
 const state=snapshot({jobs:[{id:'done',status:'completed',title:'Done',department:'Engineering'},{id:'waiting',status:'ready',title:'Waiting',department:'Engineering'}]});const a=app(state,(url)=>url==='/api/state'?state:{status:'ready',waitReason:'Folder occupied'});a.$('[data-view="requests"]').click();assert.equal(a.$('#inbox-count').textContent,'1');assert.equal(a.w.document.querySelectorAll('tbody tr').length,1);assert.ok(a.$('tbody [data-send-now="waiting"]'));a.$('tbody [data-send-now="waiting"]').click();await tick();assert.deepEqual(a.calls.find(c=>c.url==='/api/send-now').body,{id:'waiting'});a.close();
});

test('universe deletion returns to All Codex and remains restorable without deleting chats',async()=>{
 const universe={id:'ai-business',name:'New AI business',description:'',threadIds:['chat'],linkedThreadIds:['chat'],linkedProjectIds:[],jobIds:[],projectIds:[],departments:['Engineering'],customDepartments:[],managers:[],discovery:{status:'needs-context'}};let state=snapshot({threads:[{id:'chat',title:'Existing chat',department:'Engineering'}],universes:[universe],deletedUniverses:[]});
 const a=app(state,(url,b)=>{if(url==='/api/universes/delete'){state={...state,universes:b.restore?[universe]:[],deletedUniverses:b.restore?[]:[{id:universe.id,name:universe.name}]};return {id:universe.id,deleted:!b.restore}}return state});a.$('#universe-picker').value=universe.id;a.$('#universe-picker').dispatchEvent(new a.w.Event('change'));assert.match(a.$('.universe-discovery-status').textContent,/Learning your focus/);a.$('#universe-manage').click();assert.match(a.$('#universe-dialog').textContent,/Chats, files and active work stay/);a.$('#universe-delete').click();await tick();assert.equal(a.$('#universe-picker').value,'');assert.match(a.w.document.body.textContent,/Existing chat/);a.$('#universe-manage').click();a.$('[data-restore-universe]').click();await tick();assert.equal(a.$('#universe-picker').value,universe.id);assert.equal(a.calls.filter(c=>c.url==='/api/universes/delete').length,2);a.close();
});

test('questions remain visible across universes and open the global answer inbox',async()=>{
 const thread={id:'outside',title:'Approve the menu',department:'Design'},question={id:'outside-question',threadId:thread.id,method:'item/tool/requestUserInput',params:{questions:[{id:'color',question:'Which color?'}]},canAccept:true};
 const universe={id:'blank',name:'Separate business',threadIds:[],jobIds:[],projectIds:[],departments:[],customDepartments:[],managers:[]};
 const a=app(snapshot({threads:[thread],approvals:[question],universes:[universe]}));a.$('#universe-picker').value='blank';a.$('#universe-picker').dispatchEvent(new a.w.Event('change'));
...
test('switching universes clears an inspector continuation target',()=>{
