...
readline.createInterface({input:process.stdin}).on('line',line=>{
 ...
 case 'turn/steer':{const t=threads.get(p.threadId);if(t?.turn?.id!==p.expectedTurnId)return send({id:m.id,error:{message:'Turn changed'}});return ok({turnId:p.expectedTurnId});}
