...
export function discoveryProfile(data,u,threads){
 ...
 const examples=[...(u.threadIds||[]).map(id=>byId.get(id)).filter(Boolean).map(t=>({title:taskText(t.title,180),request:taskText(t.preview,500)})),...(data.jobs||[]).filter(j=>j.universeId===u.id&&!j.managerForDepartment).slice(-12).map(j=>({title:taskText(j.title,180),request:taskText(j.prompt,500)}))].slice(-16);
...
export class UniverseDiscovery{
 ...
 async tick(){
  ...
  const data=this.store.data,universes=(data.universes||[]).filter(u=>u.autoDiscover!==false);if(!universes.length)return;
  const threads=this.threads();let work;
  for(let i=0;i<universes.length;i++){
   const u=universes[(this.cursor+i)%universes.length],profile=discoveryProfile(data,u,threads),profileHash=hash(profile);u.discoveryChecks??={};
