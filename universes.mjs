...
export function saveUniverse(data,input,projects=[],threads=[]){
 ...
 Object.assign(u,{name,description,updatedAt:Date.now(),...(projectIds?{projectIds,projectPaths:projects.filter(p=>projectIds.includes(p.projectId)).map(p=>p.path)}:{}),...(threadIds?{threadIds}:{})});if(!old)data.universes.push(u);return u;
}
export function universeScope(data,id,threads=[],projects=[]){
 ...
 const threadIds=new Set(universe.threadIds);for(const j of data.jobs||[])if(j.universeId===id&&j.threadId)threadIds.add(j.threadId);
 for(const t of threads)if(paths.has(t.cwd))threadIds.add(t.id);
 // Descendants follow their real parent, even when they use a separate worktree.
 let changed=true;while(changed){changed=false;for(const t of threads)if(t.parentThreadId&&threadIds.has(t.parentThreadId)&&!threadIds.has(t.id)){threadIds.add(t.id);changed=true;}}
...
export function universeDirectory(data,threads,projects,managers=[]){return (data.universes||[]).map(u=>{const scope=universeScope(data,u.id,threads,projects);return {...u,departmentManagers:undefined,linkedThreadIds:u.threadIds,linkedProjectIds:u.projectIds,threadIds:scope.threads.map(t=>t.id),jobIds:scope.jobs.map(j=>j.id),projectIds:scope.projects.map(p=>p.projectId),departments:departmentCatalog(scope,scope.threads),managers:managers.filter(m=>m.universeId===u.id)};});}
